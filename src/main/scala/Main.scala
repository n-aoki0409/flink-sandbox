import org.apache.flink.api.common.functions.RichFlatMapFunction
import org.apache.flink.api.common.state.{ValueState, ValueStateDescriptor}
import org.apache.flink.api.java.utils.ParameterTool
import org.apache.flink.configuration.Configuration
import org.apache.flink.streaming.api.functions.co.RichCoFlatMapFunction
import org.apache.flink.streaming.api.scala._
import org.apache.flink.streaming.api.windowing.assigners.TumblingProcessingTimeWindows
import org.apache.flink.streaming.api.windowing.time.Time
import org.apache.flink.util.Collector

object Main extends App {

  val env = StreamExecutionEnvironment.getExecutionEnvironment

  val flintstones: DataStream[Person] = env.fromElements(Person("Fred", 35), Person("Wilma", 34), Person("Pebbles", 2))
  flintstones
    .keyBy(_.age)
    .flatMap(new Deduplicator)
    .filter(_.age >= 18)
    .print()

//  val control = env.fromElements("DROP", "IGNORE").keyBy(x => x)
//  val streamOfWords = env.fromElements("Apache", "DROP", "Flink", "IGNORE").keyBy((x: String) => x)
//  control
//    .connect(streamOfWords)
//    .flatMap(new ControlFunction)
//    .print()

//  val text = env.socketTextStream("localhost", 9999)
//
//  val counts = text.flatMap { _.toLowerCase.split("\\W+") filter { _.nonEmpty } }
//    .map { (_, 1) }
//    .keyBy(_._1)
//    .window(TumblingProcessingTimeWindows.of(Time.seconds(5)))
//    .sum(1)
//
//  counts.print()

//  env.fromCollection(List(
//    (1L, 3L),
//    (1L, 5L),
//    (1L, 7L),
//    (1L, 4L),
//    (1L, 2L)
//  )).keyBy(_._1)
//    .flatMap(new CountWindowAverage())
//    .print()

  env.execute()
}

case class Person(name: String, age: Int)

class Deduplicator extends RichFlatMapFunction[Person, Person] {
  private var keyHasBeenSeen: ValueState[Boolean] = _

  override def open(conf: Configuration) = {
    keyHasBeenSeen = getRuntimeContext.getState(new ValueStateDescriptor[Boolean]("keyHasBeenSeen", classOf[Boolean]))
  }

  override def flatMap(person: Person, out: Collector[Person]) = {
    if (keyHasBeenSeen.value == null) {
      out.collect(person)
      keyHasBeenSeen.update(true)
    }
  }
}

class ControlFunction extends RichCoFlatMapFunction[String, String, String] {
  private var blocked: ValueState[Boolean] = _

  override def open(conf: Configuration) = {
    blocked = getRuntimeContext.getState(new ValueStateDescriptor[Boolean]("blocked", classOf[Boolean]))
  }

  override def flatMap1(controlValue: String, out: Collector[String]): Unit = {
    blocked.update(true)
  }

  override def flatMap2(dataValue: String, out: Collector[String]): Unit = {
    if (blocked.value == null) {
      out.collect(dataValue)
    }
  }
}

class CountWindowAverage extends RichFlatMapFunction[(Long, Long), (Long, Long)] {

  private var sum: ValueState[(Long, Long)] = _

  override def flatMap(input: (Long, Long), out: Collector[(Long, Long)]): Unit = {

    // access the state value
    val tmpCurrentSum = sum.value

    // If it hasn't been used before, it will be null
    val currentSum = if (tmpCurrentSum != null) {
      tmpCurrentSum
    } else {
      (0L, 0L)
    }

    // update the count
    val newSum = (currentSum._1 + 1, currentSum._2 + input._2)

    // update the state
    sum.update(newSum)

    // if the count reaches 2, emit the average and clear the state
    if (newSum._1 >= 2) {
      out.collect((input._1, newSum._2 / newSum._1))
      sum.clear()
    }
  }

  override def open(parameters: Configuration): Unit = {
    sum = getRuntimeContext.getState(
      new ValueStateDescriptor[(Long, Long)]("average", createTypeInformation[(Long, Long)])
    )
  }
}
